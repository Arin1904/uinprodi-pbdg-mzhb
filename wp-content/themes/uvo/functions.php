...
if ( ! function_exists( 'radiantthemes_setup' ) ) :
	...
	function radiantthemes_setup() {
		 ...
		 * If you're building a theme based on uvo, use a find and replace
		 * to change 'uvo' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'uvo', get_template_directory() . '/languages' );
		...
		*/
		...
		add_theme_support( 'custom-background', $uvo_args );
		...
		function radiantthemes_editor_fonts_url() {
			...
			*/
			$rubik = esc_html_x( 'on', 'Rubik font: on or off', 'uvo' );
		...
		function radiantthemes_custom_redux_css() {
			...
			function radiantthemes_override_css_fonts_url() {
				...
				*/
				if ( 'off' !== _x( 'on', 'Google font: on or off', 'uvo' ) ) {
					$google_font_url = add_query_arg( 'family', rawurlencode( 'Rubik:400,600' ), '//fonts.googleapis.com/css' );
				}
		...
		// This example assumes your opt_name is set to uvo_theme_option, replace with your opt_name value.
		add_action( 'redux/page/uvo_theme_option/enqueue', 'radiantthemes_custom_redux_css', 2 );
...
function radiantthemes_widgets_init() {
	...
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'uvo' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'uvo' ),
	...
	if ( class_exists( 'woocommerce' ) ) {
		register_sidebar(
			array(
				'name'          => esc_html__( 'Product | Sidebar', 'uvo' ),
	...
	if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
		...
		for ( $j = 1; $j <= 4; $j++ ) {
			register_sidebar(
				array(
					'name'          => esc_html__( 'Footer | #', 'uvo' ) . $j . '',
					'id'            => 'uvo-footer-area-' . $j,
					'description'   => esc_html__( 'Add widgets here.', 'uvo' ),
					'before_widget' => '<section id="%1$s" class="widget %2$s">',
					'after_widget'  => '</section>',
	...
	register_sidebar(
		array(
			'name'          => esc_html__( 'Hamburger | Sidebar', 'uvo' ),
			'id'            => 'radiantthemes-hamburger-sidebar',
			'description'   => esc_html__( 'Add widgets for "Hamburger" menu from here. To turn it on/off please navigate to "Theme Options > Header" and select "Hamburger" for respetive header styles.', 'uvo' ),
...
function radiantthemes_scripts() {
	...
	if ( ! class_exists( 'ReduxFrameworkPlugin' ) ) {
		...
		/**
		 * Load Rubik and Poppins  Google Font when redux framework is not installed.
		 */
		...
		function radiantthemes_default_google_fonts_url() {
			...
			*/
			...
			/*
			* Translators: If there are characters in your language that are not
			* supported by Barlow Semi Condensed, translate this to 'off'. Do not translate
			* into your own language.
			*/
			$poppins = esc_html_x( 'on', 'Poppins font: on or off', 'uvo' );
	...
	if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
		...
		function radiantthemes_typekit_google_fonts_url() {
			...
			*/
			if ( 'off' !== _x( 'on', 'Google font: on or off', 'uvo' ) ) {
				$google_font_url = add_query_arg( 'family', rawurlencode( 'Rubik:400,600|Poppins:400,500,600' ), '//fonts.googleapis.com/css' );
			}
			return $google_font_url;
		}
		wp_enqueue_style(
	...
	if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
		$radiantthemes_coming_soon  = '';
		$radiantthemes_theme_option = get_option( 'uvo_theme_option' );
		if ( isset( $radiantthemes_theme_option['coming_soon_switch'] ) ) {
			$radiantthemes_coming_soon = true;
		}
	...
	wp_enqueue_script(
		'radiantthemes-retina',
		get_parent_theme_file_uri( '/assets/js/dense.js' ),
		array(),
		time(),
		true
	);
	wp_add_inline_script( 'radiantthemes-retina', "jQuery('.radiantthemes-retina img').dense({'glue' : '@'});" );
...
function radiantthemes_breadcrumbs() {
	...
	$home         = esc_html__( 'Home', 'uvo' ); // text for the 'Home' link.
	...
	if ( is_home() && is_front_page() ) {
		...
		if ( 1 === $show_on_home ) {
			echo '<div id="crumbs"><a href="' . esc_url( $home_link ) . '">' . esc_html__( 'Home', 'uvo' ) . '</a></div>';
		}
		...
		echo '<div id="crumbs"><a href="' . esc_url( $home_link ) . '">' . esc_html__( 'Home', 'uvo' ) . '</a> ' . wp_kses( $delimiter, 'rt-content' ) . ' ';
		if ( is_home() ) {
			...
			echo wp_kses( $before, 'rt-content' ) . esc_html__( 'Archive by category "', 'uvo' ) . single_cat_title( '', false ) . '"' . wp_kses( $after, 'rt-content' );
		} elseif ( is_search() ) {
			echo wp_kses( $before, 'rt-content' ) . esc_html__( 'Search results for "', 'uvo' ) . get_search_query() . '"' . wp_kses( $after, 'rt-content' );
		} elseif ( is_day() ) {
			...
			echo wp_kses( $before, 'rt-content' ) . esc_html__( 'Posts tagged "', 'uvo' ) . single_tag_title( '', false ) . '"' . wp_kses( $after, 'rt-content' );
		} elseif ( is_author() ) {
			global $author;
			$userdata = get_userdata( $author );
			echo wp_kses( $before, 'rt-content' ) . esc_html__( 'Articles posted by ', 'uvo' ) . esc_html( $userdata->display_name ) . wp_kses( $after, 'rt-content' );
		} elseif ( is_404() ) {
			echo wp_kses( $before, 'rt-content' ) . esc_html__( 'Error 404', 'uvo' ) . wp_kses( $after, 'rt-content' );
		}
		...
		if ( get_query_var( 'paged' ) ) {
			...
			echo esc_html_e( 'Page', 'uvo' ) . ' ' . get_query_var( 'paged' );
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
				echo ')';
			}
		}
...
function radiantthemes_dashboard_submenu_page() {
	add_submenu_page(
		'themes.php',
		esc_html__( 'RadiantThemes Dashboard', 'uvo' ),
		esc_html__( 'RadiantThemes Dashboard', 'uvo' ),
		'manage_options',
...
function radiantthemes_plugins_submenu_page() {
	...
	add_submenu_page(
		'themes.php',
		esc_html__( 'Radiantthemes Install Plugins', 'uvo' ),
		esc_html__( 'Radiantthemes Install Plugins', 'uvo' ),
		'manage_options',
...
function radiantthemes_custom_fonts( $array ) {
	$theme_options = get_option( 'uvo_theme_option' );
	$font_names    = array();
	for ( $i = 1; $i <= 50; $i++ ) {
	...
	$array = array(
		esc_html__( 'Custom Fonts', 'uvo' ) => $final_custom_fonts,
	);
	return $array;
};

// add the filter.
add_filter( 'redux/uvo_theme_option/field/typography/custom_fonts', 'radiantthemes_custom_fonts', 10, 1 );
...
function radiantthemes_add_themify_icons_tab( $tabs = array() ) {
	...
	$tabs['my-custom-icons'] = array(
		...
		'label'         => esc_html__( 'Themify Icons', 'uvo' ),
		'labelIcon'     => 'ti ti-user',
		'prefix'        => 'ti-',
		'displayPrefix' => 'ti',
		'url'           => 'https://uvo.radiantthemes.com/wp-content/themes/uvo/assets/css/radiantthemes-all.min.css',
		'icons'         => $new_icons,
		'ver'           => '1.0.0',
	);
...
function radiantthemes_fw_ext_backups_demos( $demos ) {
	$demos_array = array(
		'uvo' => array(
			'title'        => __( 'uvo', 'uvo' ),
			'screenshot'   => 'https://uvo.radiantthemes.com/wp-content/themes/uvo/screenshot.png',
			'preview_link' => 'https://uvo.radiantthemes.com/',
		),
	);

	$download_url = 'https://api.radiantthemes.com/demo-data/uvo/';
...
function radiantthemes_get_like_count( $type = 'likes' ) {
...
function radiantthemes_process_like() {
	...
	add_action( 'template_redirect', 'radiantthemes_process_like' );
		// ---- Add buttons to top of post content
function radiantthemes_post_likes( $content ) {
				...
				<ul class="likes">
					<li class="likes__item likes__item--like">
						<a href="<?php echo add_query_arg( 'post_action', 'like' ); ?>">
							Like (<?php echo radiantthemes_get_like_count( 'likes' ); ?>)
						</a>
					</li>
...
function rt_price_action() {
	echo '<div class="text-price"><h6>';
	esc_html_e( 'Product Price', 'uvo' );
	echo '</h6></div>';
};
add_action( 'woocommerce_single_product_summary', 'rt_price_action', 11 );
remove_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_price', 10 );
add_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_price', 12 );

add_filter( 'gettext', 'rt_translate_tag_taxonomy' );
add_filter( 'ngettext', 'rt_translate_tag_taxonomy' );

function rt_translate_tag_taxonomy( $translated ) {

	// if ( is_product() ) {
	// This will only trigger on the single product page
	// $translated = str_ireplace('tag', 'Heighlight', $translated);
	// }
...
function override_default_address_checkout_fields( $address_fields ) {
	...
	$address_fields['email']['placeholder']      = 'Email address';
...
function placeholder_comment_form_fields( $fields ) {
	$replace_author = __( 'Your name', 'uvo' );
	$replace_email  = __( 'Your email', 'uvo' );
	$replace_url    = __( 'Your website', 'uvo' );
	$fields['url']  = '<p class="comment-form-url"><label for="url">' . __( 'Website', 'uvo' ) . '</label>' .
	'<input id="url" name="url" type="text" placeholder="' . $replace_url . '" value="' . esc_attr( $commenter['comment_author_url'] ) .
	'" size="30" /></p>';

	$fields['author'] = '<p class="comment-form-author">' . '<label for="author">' . __( 'Name', 'uvo' ) . '</label> ' . ( $req ? '<span class="required">*</span>' : '' ) .
	...
	$fields['email'] = '<p class="comment-form-email"><label for="email">' . __( 'Email', 'uvo' ) . '</label> ' .
...
/**
 * Change the reply title tag to H4
 *
 * @param array $args Comment Arguments.
 * @return array
 */
function rt_commentform_title( $args ) {
	$args['title_reply_before'] = '<h4 id="reply-title" class="comment-reply-title">';
	$args['title_reply_after']  = '</h4>';
	$args['class_container']    = 'comment-respond rt-comment-respond';
	return $args;
}
add_filter( 'comment_form_defaults', 'rt_commentform_title' );

/**
 * Reorder and edit Comment form fields
 *
 * @param array $fields Comment fields.
 * @return array
 */
function radiantthemes_move_comment_field_to_bottom( $fields ) {
	unset( $fields['comment'] );
	$cookies_field = $fields['cookies'];
	unset( $fields['cookies'] );
	$fields['comment'] = '<p class = "comment-form-comment"><textarea id = "comment" name = "comment" aria-required = "true" placeholder = "' . esc_attr__( 'Your Comment *', 'uvo' ) . '"></textarea></p>';
	$fields['cookies'] = $cookies_field;
	return $fields;
}
add_filter( 'comment_form_fields', 'radiantthemes_move_comment_field_to_bottom' );

if ( ! function_exists( 'woocommerce_template_loop_product_title' ) ) {
	/**
	 * Show the product title in the H6.
	 *
	 * @return void
	 */
	function woocommerce_template_loop_product_title() {
		echo '<h6 class="woocommerce-loop-product__title">' . get_the_title() . '</h6>';
	}
}
...
function radiantthemes_star_rating() {
	...
	echo '<div class="star-rating"><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"><strong  class="rating">' . esc_attr( $average ) . '</strong> ' . __( 'out of 5', 'uvo' ) . '</span></div>';
}
