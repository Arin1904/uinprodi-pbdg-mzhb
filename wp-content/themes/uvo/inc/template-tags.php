...
if ( ! function_exists( 'radiantthemes_global_var' ) ) {
	 ...
	 *
	 * @param  [type] $uvo_opt_one   description.
	 * @param  [type] $uvo_opt_two   description.
	 * @param  [type] $uvo_opt_check description.
	...
	function radiantthemes_global_var(
		...
		global $uvo_theme_option;
		if ( $uvo_opt_check ) {
			if ( isset( $uvo_theme_option[ $uvo_opt_one ][ $uvo_opt_two ] ) ) {
				return $uvo_theme_option[ $uvo_opt_one ][ $uvo_opt_two ];
			}
		} else {
			if ( isset( $uvo_theme_option[ $uvo_opt_one ] ) ) {
				return $uvo_theme_option[ $uvo_opt_one ];
			}
		}
	}
}
				...
				printf(
					...
					printf(
						wp_kses(
    						...
    						<span class="byline"> ' . esc_html__( 'By', 'uvo' ) . ' ' . $byline . '</span>', 'rt-content'
						)
					);
			...
			if ( 'post' === get_post_type() ) {
				if ( is_single() ) {
					...
					if ( true == radiantthemes_global_var( 'display_categries', '', false ) ) :
						$categories_list = get_the_category_list( __( ', ', 'uvo' ) );
						if ( $categories_list && radiantthemes_categorized_blog() ) {
									printf(
										wp_kses( '<span class="category"><span class="ti-direction-alt"></span> ' . esc_html__( 'In', 'uvo' ) . '', 'rt-content' ) .
										/* translators: used between list items, there is a space after the comma */
										esc_html( ' %1$s' ) .
...
function radiantthemes_search_form_widget( $form ) {
    ...
    <label>
        <input type="text" class="search-field" placeholder="' . esc_attr__( 'Search Site', 'uvo' ) . '" value="' . get_search_query() . '" name="s">
    </label>
    <button type="submit" value="' . esc_attr__( 'Search', 'uvo' ) . '">Search</button>
    </form>';
    return $form;
}
...
function radiantthemes_wc_add_to_cart_message( $message, $products ) {
	...
	foreach ( $products as $product_id => $qty ) {
		$titles[] = ( $qty > 1 ? absint( $qty ) . ' &times; ' : '' ) . sprintf( _x( '&ldquo;%s&rdquo;', 'Item name in quotes', 'uvo' ), wp_strip_all_tags( get_the_title( $product_id ) ) );
		$count   += $qty;
	}

	$titles = array_filter( $titles );

	$added_text = sprintf( _n( '%s has been added to your cart.', '%s have been added to your cart.', $count, 'uvo' ), wc_format_list_of_items( $titles ) );

	// Output success messages.
	if ( 'yes' === get_option( 'woocommerce_cart_redirect_after_add' ) ) {
		$return_to = apply_filters( 'woocommerce_continue_shopping_redirect', wc_get_raw_referer() ? wp_validate_redirect( wc_get_raw_referer(), false ) : wc_get_page_permalink( 'shop' ) );

		$message = sprintf( '<div class="cart-added"><div class="cart-added-text">%s</div><div class="cart-added-button"><a href="%s" class="rt-button-added-cart button wc-forward">%s</a></div></div>', $added_text, esc_url( $return_to ), esc_html__( 'Continue Shopping', 'uvo' ) );
	} else {
		$message = sprintf( '<div class="cart-added"><div class="cart-added-text">%s</div><div class="cart-added-button"><a href="%s" class="rt-button-added-cart button wc-forward">%s</a></div></div>', $added_text, esc_url( wc_get_page_permalink( 'cart' ) ), esc_html__( 'View Cart', 'uvo' ) );
	}
