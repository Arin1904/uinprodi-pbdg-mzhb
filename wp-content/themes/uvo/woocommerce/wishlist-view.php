...
?>
			...
			<th class="product-name">
				<span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', __( 'Product Name', 'uvo' ) ); ?></span>
			</th>
			...
			<?php if ( $show_price ) : ?>
				...
				<th class="product-price">
					<span class="nobr">
						<?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', __( 'Unit Price', 'uvo' ) ); ?>
					</span>
				</th>
...
?>
			...
			<?php if ( $show_stock_status ) : ?>
				...
				<th class="product-stock-status">
					<span class="nobr">
						<?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', __( 'Stock Status', 'uvo' ) ); ?>
					</span>
				</th>
...
?>
		...
		if ( count( $wishlist_items ) > 0 ) :
				...
				if ( $product && $product->exists() ) :
					...
					<tr id="yith-wcwl-row-<?php echo esc_textarea( $item['prod_id'] ); ?>" data-row-id="<?php echo esc_textarea( $item['prod_id'] ); ?>">
						...
						<td class="product-remove">
							<div>
								<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ); ?>" class="remove remove_from_wishlist" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'uvo' ) ); ?>"><span class="ti-close"></span></a>
							</div>
						</td>
						<?php endif; ?>
						...
						<?php if ( $show_price ) : ?>
							<td class="product-price">
								...
								echo esc_textarea( $base_product ? $product->get_price_html() : apply_filters( 'yith_free_text', __( 'Free!', 'uvo' ), $product ) );
								?>
							</td>
						<?php endif ?>
						...
						<?php if ( $show_stock_status ) : ?>
							<td class="product-stock-status">
								<?php echo esc_textarea( $stock_status == 'out-of-stock' ? '<span class="wishlist-out-of-stock">' . __( 'Out of Stock', 'uvo' ) . '</span>' : '<span class="wishlist-in-stock">' . __( 'In Stock', 'uvo' ) . '</span>' ); ?>
							</td>
						<?php endif ?>
						...
						<td class="product-add-to-cart">
							...
							if ( $show_dateadded && isset( $item['dateadded'] ) ) :
								echo '<span class="dateadded">' . sprintf( __( 'Added on : %s', 'uvo' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
							endif;
							?>
							...
							<select class="change-wishlist selectBox">
								<option value=""><?php esc_html_e( 'Move', 'uvo' ); ?></option>
								<?php
								foreach ( $users_wishlists as $wl ) :
									...
									<option value="<?php echo esc_attr( $wl['wishlist_token'] ); ?>">
										...
										if ( $wl['wishlist_privacy'] == 1 ) {
											$wl_privacy = __( 'Shared', 'uvo' );
										} elseif ( $wl['wishlist_privacy'] == 2 ) {
											$wl_privacy = __( 'Private', 'uvo' );
										} else {
											$wl_privacy = __( 'Public', 'uvo' );
										}
							...
							<?php if ( $is_user_owner && $repeat_remove_button ) : ?>
								<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ); ?>" class="remove_from_wishlist button" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'uvo' ) ); ?>"><?php _e( 'Remove', 'uvo' ); ?></a>
							<?php endif; ?>
						</td>
					<?php endif; ?>
					</tr>
			...
			?>
			<tr>
				<td colspan="<?php echo esc_attr( $column_count ); ?>" class="wishlist-empty"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', __( 'No products were added to the wishlist', 'uvo' ) ); ?></td>
			</tr>
			<?php
		endif;
	...
	<div id="ask_an_estimate_popup">
		<form action="<?php echo esc_url( $ask_estimate_url ); ?>" method="post" class="wishlist-ask-an-estimate-popup">
			<?php if ( ! is_user_logged_in() ) : ?>
				<label for="reply_email"><?php echo apply_filters( 'yith_wcwl_ask_estimate_reply_mail_label', __( 'Your email', 'uvo' ) ); ?></label>
			...
			<button class="btn button ask-an-estimate-button ask-an-estimate-button-popup" >
				...
				<?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', __( 'Ask for an estimate', 'uvo' ) ); ?>
			</button>
		</form>
	</div>
<?php endif; ?>
